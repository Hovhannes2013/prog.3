...
class gishatich{
    ...
    move() {
        ...
        if (cord) {
            ...
            this.energy-=2; //energian qchanum  e 2ov 
     ...
     eat() {
        //հետազոտում է շրջակայքը, որոնում է խոտ
    ...
    mul() {
        ...
        if (cord){
            ...
            // this.multiply++;
            //ստեղծում է նոր օբյեկտ (այստեղ խոտակեր) 
            //և տեղադրում է այն գիշատիչններ զանգվածի մեջ
    ...
    die() {
        ...
        //ջնջում է ինքն իրեն գիշատիչներ զանգվածից
        for (var i in gishArr) {
...
class hzor {
    ...
    teleport(){
        this.newDirections();
        var cord = random(this.directions);
        let x = cord[0];
        let y = cord[1];
        if (x >= 0 && x < matrix[0].length && y >= 0 && y < matrix.length)    //stugum e vor canvasic durs chga
        {
            let r = matrix[y][x];
            if(r == 1)
        {
